...
@interface ZXWebViewController ()<UIWebViewDelegate,ZXEmptyViewControllerDelegate>
...
- (void)setupNav
{
    ...
    NSRange rangeDuiBa = [self.URLString rangeOfString:@"duiba.com.cn" options:NSAnchoredSearch];
    if(rangeDuiBa.location == NSNotFound)
    { //屏蔽兑吧域名，兑吧界面不展示分享
...
- (void)loadRequestWebPageWithURLString
{
    ...
    {
        NSString * string= [self.self.URLString stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
...
-(void)setNavTitle:(NSDictionary *)dic{
    ...
    //    [self.webView stringByEvaluatingJavaScriptFromString:@"(function (text) {\n          alert(text);\n        })('a')"];
}
...
////注册返回事件获取；
//- (void)backAction:(NSDictionary *)dic
//{
//    self.customH5GoBackDic = [dic copy];
//}
