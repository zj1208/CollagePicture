...
- (BOOL)jq_ChecklookupTable:(NSString *)tableName dicOrModel:(id)parameters whereFormat:(NSString *)format, ...
{
    NSArray *arr = [self jq_lookupTable:tableName dicOrModel:parameters whereFormat:format];
    if (arr.count > 0) {
        return YES;
    }
    return NO;
