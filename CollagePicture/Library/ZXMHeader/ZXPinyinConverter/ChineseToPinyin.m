...
//排序各分组里的小分组，按字符串比较大小排序歌曲名
+ (NSMutableArray*)sortName:(NSMutableArray *)aArray
{
    for (int i = 0; i<[aArray count]; i++)
    {
        for (int j = 0; j<[aArray count]-i-1; j++)
        {
            NSString * str1 =[ChineseToPinyin pinyinFromChiniseString:[aArray objectAtIndex:j]];
            NSString * str2 =[ChineseToPinyin pinyinFromChiniseString:[aArray objectAtIndex:j+1]];
            if([str1 compare:str2 options:NSCaseInsensitiveSearch] == NSOrderedDescending )//str必须转换为字母
            {
                [aArray exchangeObjectAtIndex:j withObjectAtIndex:j+1]; //按升序的排序
            }
        }
    }
    return aArray;
...
{
    NSMutableArray *sectionArray = [NSMutableArray arrayWithCapacity:26];
    ...
    for (char ch ='A'; ch<='Z'; ch++)
    {
        ...
        {
            NSString *title = [ChineseToPinyin pinyinFromChiniseString:[titleArray objectAtIndex:i]];
            char firstChar = [ChineseToPinyin sortSectionTitle:title];
            if (firstChar ==ch)
            {
                [tempArray addObject:[titleArray objectAtIndex:i]];
            }
        }
        
        [sectionArray addObject:[self sortName:tempArray]];
    }
  
    //创建一个按字母排序，各组里字符串又排序的字典
   NSDictionary *dic = [NSDictionary dictionaryWithObjects:sectionArray forKeys:[ChineseToPinyin keyArray]];
