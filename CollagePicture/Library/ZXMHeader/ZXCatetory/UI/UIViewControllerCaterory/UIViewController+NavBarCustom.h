//
//  UIViewController+NavBarCustom.h
//  ShiChunTang
//
...
@interface UIViewController (NavBarCustom)
 ...
 * 例如： 控制item显示及边距：
    self.navigationItem.leftBarButtonItems = [self zhNavigationItem_leftOrRightItemReducedSpaceToMaginWithItems:@[self.personCenterBarItem]];
 ...
 *  @brief set all navigationBar 's 系统返回按钮为没有文字； 把文字移至看不到; iOS11无法移出title，只能小范围运动一点距离；
 ...
 需要调用zhNavigationItem_titleCenter方法解决返回按钮文本为空；按钮宽度也会变小；
 *
 */
...
//每个navigationController有一个navigationBar，单独设置
...
- (void)zhNavigationBar_BackgroundAlpah:(CGFloat)alpha navigationBar:(UINavigationBar *)navigationBar;
